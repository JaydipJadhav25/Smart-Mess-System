  ...
  useEffect(() => {
    async function fetchApplication() {
      ...
      try {
        ...
        const response = await axiosInstance.get(`/admin/application/review/${id}`);
...
const accepteApplication = async(applicationId : any) =>{
     if (!id) return;
   try {
    setIsLoading(true);
    const response = await axiosInstance.get(`/admin/application/update/${applicationId}`)
    
    console.log("user accepeted application successfully : " , response.data);
    alert("successfully Accepted Application")
    //back to admin page
    navigate(-1);
     
   } catch (error) {
    alert("error occured accepte application")
   }finally{
    setIsLoading(false);
   }
}
...
const delateApplication = async(applicationId : any) =>{
     if (!id) return;
   try {
    setIsLoading(true);
    const response = await axiosInstance.delete(`/admin/application/delete/${applicationId}`)
    ...
    alert("error occured Delete application")
   }finally{
    setIsLoading(false);
   }
}
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="border-b border-orange-200 pb-4 mb-6">
        <h2 className="text-2xl font-bold text-orange-600">Review Application</h2>
      ...
      <div className="flex justify-between pt-8">
        ...
        className="px-8 py-3 bg-green-500 text-white rounded-md hover:bg-green-600 font-medium">
        {
         ...
         <button
        hidden={!data.isAccepted}
        onClick={()=> delateApplication(data._id)}
